  ...
  object.traverse((child) => {
    if (child.isMesh) {
      ...
      if (child.material) {
        ...
        const hasTexture = materials.some((material) => material.map || material.emissiveMap || material.normalMap || material.roughnessMap);
...
async function loadOBJ(files) {
  ...
  if (mtlFile) {
    ...
    const rewrittenMtlText = mtlText
      .replace(/\\/g, "/")
      .replace(
        /(map_Kd|map_Ka|map_bump|bump)\s+([^\s]+)/gi,
        (match, key, value) => {
    ...
    const mtlUrl = URL.createObjectURL(
      new Blob([rewrittenMtlText], { type: "text/plain" }),
    );
    objectUrlsToRevoke.push(mtlUrl);

    const rewrittenObjText = objText.replace(
      /mtllib\s+([^\s]+)/i,
      `mtllib ${mtlUrl}`,
    );
    const rewrittenObjUrl = URL.createObjectURL(
      new Blob([rewrittenObjText], { type: "text/plain" }),
    );
